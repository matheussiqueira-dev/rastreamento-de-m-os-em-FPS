...
const CinematicGenerator: React.FC<CinematicGeneratorProps> = ({ onClose }) => {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      setStatus('Iniciando Motor Cinemático...');

      const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
      const base64Data = selectedImage.split(',')[1];
      const mimeType = selectedImage.split(';')[0].split(':')[1];

      setStatus('Rascunhando Narrativa Visual...');
      
      let operation = await ai.models.generateVideos({
        model: 'veo-3.1-fast-generate-preview',
        prompt: 'Uma animação cinematográfica de alta qualidade desta cena com movimento sutil e atmosfera.',
        image: {
      ...
      const loadingMessages = [
        'Calculando trajetórias de quadros...',
        'Sintetizando texturas temporais...',
        'Refinando dinâmica atmosférica...',
        'Renderizando sequência final...',
        'Finalizando obra-prima...'
      ];
      ...
      if (downloadLink) {
        setStatus('Sequência adquirida.');
        const finalUrl = `${downloadLink}&key=${process.env.API_KEY}`;
        setGeneratedVideoUrl(finalUrl);
      } else {
        throw new Error('Falha na geração - nenhum link retornado.');
      }
    } catch (err: any) {
      console.error(err);
      setStatus(`Erro: ${err.message || 'Falha ao renderizar sequência.'}`);
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 backdrop-blur-2xl animate-in fade-in duration-300">
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-neutral-900 border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/40">
          <div>
            <h2 className="text-2xl font-black text-cyan-400 tracking-tighter uppercase italic">Gerador de Sequência Cinemática</h2>
            <p className="text-white/40 text-xs">Transforme capturas estáticas em sequências animadas</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">1. Fonte da Captura</label>
              <div 
              ...
              >
                {selectedImage ? (
                  ...
                  <div className="text-center p-4">
                    ...
                    <p className="text-white/60 text-sm font-bold">Selecionar Quadro da Cena</p>
                    <p className="text-white/20 text-[10px] mt-1 uppercase">JPG, PNG Suportados</p>
                  </div>
                )}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest">2. Configuração Temporal</label>
            ...
            >
              {isGenerating ? 'Renderizando...' : 'INICIAR GERAÇÃO'}
            </button>
          </div>

          <div className="bg-black/50 rounded-2xl border border-white/5 flex flex-col items-center justify-center min-h-[300px] overflow-hidden">
            {isGenerating ? (
              ...
              <div className="relative w-full h-full flex flex-col">
                 ...
                 <div className="p-4 bg-black/80 flex justify-between items-center border-t border-white/10">
                    <span className="text-[10px] font-bold text-cyan-400 uppercase">Sequência Renderizada com Sucesso</span>
                    <a 
                      href={generatedVideoUrl} 
                      download="render_cinematico.mp4"
              ...
              <div className="text-center p-8 text-white/20 space-y-2">
                ...
                <p className="text-sm font-bold uppercase tracking-widest">Aguardando Entrada...</p>
                <p className="text-[10px]">Sua sequência aparecerá aqui.</p>
              </div>
            )}
          </div>
        </div>
